...
class RepoListAdapter : ListAdapter<Repo, RepoListAdapter.ViewHolder>(DiffCallback()) {
    ...
    inner class ViewHolder(
        ...
        fun bind(item: Repo) {
            binding.tvRepositorieName.text = item.name
            binding.tvRepositorieDescription.text = item.description
            binding.tvRepositorieLanguage.text = item.language
            binding.chipStar.text = item.stargazersCount.toString()
            Glide.with(binding.root.context)
                .load(item.owner.avatarURL).into(binding.ivPicOwner)
        }
    }
}
